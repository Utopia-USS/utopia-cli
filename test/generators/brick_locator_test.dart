...
void main() {
  group('BrickLocator', () {
    ...
    test('uses package_config source root for global path or git activations', () {
      final temp = Directory.systemTemp.createTempSync('utopia_cli_brick_locator_test_');
      addTearDown(() {
      ...
      final sourcePackage = Directory(p.join(temp.path, 'git', 'utopia_cli'))..createSync(recursive: true);
      ...
      final packageConfigDir = Directory(p.join(globalPackage.path, '.dart_tool'))..createSync(recursive: true);
      File(p.join(packageConfigDir.path, 'package_config.json')).writeAsStringSync('''
{
  "configVersion": 2,
  "packages": [
    {
